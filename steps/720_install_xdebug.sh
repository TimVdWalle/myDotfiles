...
if ! /usr/local/bin/pecl list | grep -q xdebug; then
  # Check architecture and execute appropriate commands
  if [ "$ARCHITECTURE" = "x86_64" ]; then
    # Commands for Intel
    print_info "Detected Intel architecture. Running Intel-specific commands."
    execute "brew install php" "Installing php"
    execute "pecl install xdebug" "Installing xdebug"
  elif [ "$ARCHITECTURE" = "arm64" ]; then
    # Commands for M1/M2/Mx
    print_info "Detected M1/M2/Mx architecture. Running M1/M2/Mx-specific commands."
    # Currently, the same commands as for Intel are used. Adjust if necessary.
    execute "brew install php" "Installing php"
    execute "arch -arm64 sudo pecl install xdebug" "Installing xdebug"
  else
    print_error "Unknown architecture detected."
    execute "brew install php" "Installing php"
    execute "pecl install xdebug" "Installing xdebug"
  ...
  #read waiting
    ...
    print_success "Xdebug is already installed."
fi
